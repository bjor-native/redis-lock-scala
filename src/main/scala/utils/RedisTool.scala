...
object RedisTool {
   ...
   */
  def tryGetDistributedLock(lockKey: String, requestId: String, expireTime: Duration, isPersist: Boolean = false): Unit = {
   ...
   * @return
   */
  def releaseDistributedLock(lockKey: String, requestId: String, expireTime: Int = 1, isPersist: Boolean = false): Boolean = {
    CacheManager.redisClientPool.withClient(
      client => {
        val redisKeyPrefix = CacheManager.getRedisKeyPrefix(isPersist)
        //client.select(CacheManager.redisDBNum)
